...
namespace NetCoreFrameworkWorkShop.Core.Utilities.Security.JWT
{
    public class JwtHelper : ITokenHelper
    {
        ...
        {
            var securityKey = SecuirtyKeyHelper.CreateSecurityKey(_tokenOptions.SecurityKey);
            var signingCredentials = SigningCredentialsHelper.CreateSigningCredentials(securityKey);
            var jwt = CreateJwtSecuityToken(_tokenOptions, user, signingCredentials, operationClaims);
            var jwtSecurityTYokenHelper = new JwtSecurityTokenHandler();
            var token = jwtSecurityTYokenHelper.WriteToken(jwt);
        ...
        private IEnumerable<Claim> SetClaims(User user,List<OperationClaim> operationClaims)
        {
            ...
            claims.AddRoles(operationClaims.Select(t => t.Name).ToArray());
